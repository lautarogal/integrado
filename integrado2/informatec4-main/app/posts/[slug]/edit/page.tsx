...
// Eliminamos la interfaz EditPostPageProps y usamos useParams en su lugar
...
export default function EditPostPage() {
  ...
  // Cargar categorías y datos de la publicación
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          categories: postData.categories.map((cat: any) => cat.category.id),
          pageLayout: postData.pageLayout || 'default',
          pageTheme: postData.pageTheme || 'light',
        });
      } catch (err) {
  ...
  useEffect(() => {
    if (originalPost && user && !loading) {
      // Solo el autor o un administrador pueden editar
      if (originalPost.author.id !== user.id && user.role !== 'ADMIN') {
  ...
  }, [originalPost, user, loading, router, slug]);
                ...
                <div className="pt-5 flex justify-between">
                  ...
                  <div className="flex space-x-3">
                    ...
                    <button
                      type="submit"
                      disabled={submitting}
                      className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-[#3d4fd6] hover:bg-[#5e84ff] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#3d4fd6]"
                    >
                      {submitting ? (
                        <>
